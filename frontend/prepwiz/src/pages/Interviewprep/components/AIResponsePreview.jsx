import { LuCopy, LuCheck, LuCode, LuCopyCheck } from "react-icons/lu";
      ...
      <div className="text-[14px] prose prose-slate dark:prose-invert max-w-none">
        <ReactMarkdow
          ...
          components={{
            ...
            ol({ children }) {
              return (
                <ol className="list-decima pl-6 space-y-2 my-4">{children}</ol>
              );
            },
            li({ children }) {
            ...
            img({ src, alt }) {
              return (
                <img src={src} alt={allt} className="my-4 max-w-full rounded" />
              );
            },
          }}
        >
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200">
        ...
        >
          {copied ? (
            ...
            <LuCopy
              className="absolute -top-2 right-0 bg-black text-white text-xs rounded-md px-2 py-1 opacity-80 group-hover:opacity-100 transition"
              size={16}
            />
          )}
